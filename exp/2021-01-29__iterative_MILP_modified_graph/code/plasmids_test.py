...
sample_dir = '/home/aniket/python_scripts/Plasmids-Optimization/exp/2019-05-07__iterative_MILP_unweighted/test_examples/'
output_dir = '/home/aniket/python_scripts/Plasmids-Optimization/exp/2019-05-07__iterative_MILP_unweighted/test_output/' 
...
while len(seeds_set) > 0:
	...
	contigs_ext = {}
	ceil = {}
	contigs, contigs_ext, ceil = plasmids_preprocessing.contig_vars(m, contigs_dict, contigs, contigs_ext, ceil, nplasmids)
	...
	for p in contigs_ext:
		for extr in contigs_ext[p]:
			...
			m.addConstr(expr <= math.ceil(contigs_dict[c]['Read_depth']), "consistency")
			constraint_count += 1
	...
	while circular == 1:
		...
		for p in links:
			...
			#print(soln_ext_dict)			
			solution_seq[p], contig_chain = plasmids_postprocessing.get_seq(solution_links[p], soln_ext_dict[p], contigs_dict, contigs)			
		#print("Same dict: ", soln_ext_dict)		
		circ_seq = {}
		k = 1 
...
#_______________
	...
	for p in links:
		...
		if plasmid_length[p] >= 10 and plasmid_gd[p] >= 0.3:
			output_file.write(">plasmid_"+str(n_iter)+"\t"+"length="+str(plasmid_length[p])+"\t"+"gene_density="+str(plasmid_gd[p])+"\t"+"mean_read_depth="+str(plasmid_rd[p])+"\n")
			if len(solution_seq[p]) != 0:
				output_file.write(solution_seq[p]+"\n")
			else:
				c = contig_chain[0][:-1]
				output_file.write(contigs_dict[c]['Seq']+"\n")	
			...
			questionable_file.write(">plasmid_"+str(q_iter)+"\t"+"length="+str(plasmid_length[p])+"\t"+"gene_density="+str(plasmid_gd[p])+"\t"+"mean_read_depth="+str(plasmid_rd[p])+"\n")	
	...
	for p in contigs:
		...
		#for extr in contigs_ext[p]:
		#	print(extr, contigs_ext[p][extr].x)				
