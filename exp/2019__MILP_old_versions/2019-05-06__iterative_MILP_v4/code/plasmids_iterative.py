...
output_dir = '/home/aniket/python_scripts/Plasmids-Optimization/exp/2019-05-06__iterative_MILP_v4/output/' 
...
while len(seeds_set) > 0:
	...
	for p in rd_diff:
		for c in rd_diff[p]:
			if c in rd_diff[p]:
				m.addConstr(rd_diff[p][c] >= (1 - contigs_dict[c]['Read_depth']/rd_graph)*contigs[p][c], "rd-diff_lbd1")
...
#_______________
	...
	for p in contigs:
		for c in contigs[p]:
			if contigs[p][c].x == 1:
				contigs_dict[c]['Read_depth'] = max(0, contigs_dict[c]['Read_depth'] - rd_graph)
