...
def contig_vars(m, contigs_dict, contigs, degree, contigs_ext, ceil, nplasmids):
	for p in range(nplasmids):
		...
		for c in contigs_dict:
			contigs[p][c] = m.addVar(vtype=GRB.INTEGER, name='contig-'+c+'_plasmid-'+str(p))
			degree[p][c] = m.addVar(vtype=GRB.INTEGER, name='degree-'+c+'_plasmid-'+str(p))
			contigs_ext[p][(c, 'h')] = m.addVar(vtype=GRB.INTEGER, name='contigext-'+c+'h'+'_plasmid-'+str(p))
			contigs_ext[p][(c, 't')] = m.addVar(vtype=GRB.INTEGER, name='contigext-'+c+'t'+'_plasmid-'+str(p))			
