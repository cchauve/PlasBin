...
output_dir = '/home/aniket/python_scripts/Plasmids-Optimization/exp/2019-05-07__iterative_MILP_unweighted/feasibility_check/' 
...
for line in greedy_results:
	...
	#Greedy solution constraints
	for p in contigs: 		
		for c in chain:
			c = c[:-1]
			#print(c, contigs_dict[c]['GC_cont'], contigs_dict[c]['Gene_coverage'], contigs_dict[c]['Length'])	
			m.addConstr(contigs[p][c] == 1, "greedy-solution")
...
#_______________
	...
	for p in links:
		...
		if plasmid_length[p] >= 1500 and plasmid_gd[p] >= 0.3:
			...
			if len(solution_seq[p]) != 0:							#if length of contig chain is more than one
			...
			for x in contig_chain:
				c = x[:-1]
				contigs_file.write("# "+c+"\t"+str(contigs_dict[c]['Gene_coverage'])+"\t"+str(contigs_dict[c]['Read_depth'])+"\t"+str(contigs_dict[c]['Length'])+"\n")				
			contigs_file.write("\n")
