...
output_dir = '/home/aniket/python_scripts/Plasmids/exp/2019-03-07__iterative_MILP/output/' 
...
while len(seeds_set) > 0:
	...
	for p in links:
		...
		if plasmid_length[p] >= 10000:
			output_file.write(">plasmid_"+str(n_iter)+"\t"+"length="+str(plasmid_length[p])+"\t"+"mean_read_depth="+str(mean_rd[p].x)+"\n")
		...
		for c in contigs[p]:
			if contigs[p][c] == 1:
				...
				if contigs_dict[c]['Read_depth'] <= 0.05:
					#print("Deleted contig", c)
					del(contigs_dict[c])				
					#del(contigs_ext[p][(c, 'h')])
					#del(contigs_ext[p][(c, 't')])
					#if (('52', 'h'), ('36', 't')) in links_list:
					#	print("Edge found")

					for e in links_list:						
						if (c, 'h') in e or (c, 't') in e:
							#print(e)
							#print("\nBefore removal:",len(links_list),"\n")
							#print(links_list)
							links_list.remove(e)
							#print("After removal",len(links_list),"\n")
							#print(links_list)
				#	m.addConstr(contigs[p][c] == 0, "No-read-depth-left")
				#	m.addConstr(contigs_ext[p][(c, 'h')] == 0, "No-read-depth-left_h-ext")
				#	m.addConstr(contigs_ext[p][(c, 't')] == 0, "No-read-depth-left_t-ext")
	#for k in contigs_dict:
	#	print(k)			
	#print(links_list)
	#print(len(contigs_dict))			